...
export default async function handler(request: NodeRequest, response: NodeResponse) {
  try {
    ...
    if (request.method === 'GET') {
      ...
      const rows = await dbClient`SELECT id, order_number AS "orderNumber", customer_name AS "customerName", phone, email, city, address, notes, products, subtotal, delivery_fee AS "deliveryFee", total, payment_method AS "paymentMethod", status, created_at AS "createdAt", updated_at AS "updatedAt" FROM orders ORDER BY created_at DESC`
    ...
    const customer = await dbClient`INSERT INTO customers (name, phone, email, city, address) VALUES (${body.customerName}, ${body.phone}, ${body.email || null}, ${body.city}, ${body.address}) ON CONFLICT (phone) DO UPDATE SET name = EXCLUDED.name, email = EXCLUDED.email, city = EXCLUDED.city, address = EXCLUDED.address, updated_at = NOW() RETURNING id`
    const order = await dbClient`INSERT INTO orders (order_number, customer_id, customer_name, phone, email, city, address, notes, products, subtotal, delivery_fee, total, payment_method, status) VALUES (${orderNumber}, ${customer[0].id}, ${body.customerName}, ${body.phone}, ${body.email || null}, ${body.city}, ${body.address}, ${body.notes || null}, ${JSON.stringify(items)}::jsonb, ${subtotal}, ${deliveryFee}, ${total}, ${body.paymentMethod || 'cash_on_delivery'}, 'Nouvelle') RETURNING id, order_number AS "orderNumber", total, status, created_at AS "createdAt"`
